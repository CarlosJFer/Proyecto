...
const procesarDatosExcel = (datosExcel) => {
  ...
  // Análisis por contratación
  const porContratacion = agruparPorCampo(datosExcel, 'TIPO_CONTRATACION');
  const agentesPorContratacion = Object.entries(porContratacion).map(([tipo, cantidad]) => ({
    tipo,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  
  // Análisis por función
  const porFuncion = agruparPorCampo(datosExcel, 'FUNCION');
  const agentesPorFuncion = Object.entries(porFuncion).map(([funcion, cantidad]) => ({
    funcion,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  ...
  const agentesPorEscalafon = Object.entries(porEscalafon).map(([escalafon, cantidad]) => ({
    escalafon,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  ...
  datosExcel.forEach(registro => {
    const rango = calcularRangoEdad(registro.FECHA_NACIMIENTO);
    edades[rango] = (edades[rango] || 0) + 1;
  });
  const agentesPorRangoEdad = Object.entries(edades).map(([rango, cantidad]) => ({
    rango,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  ...
  const antiguedades = {};
  datosExcel.forEach(registro => {
    const rango = calcularAntiguedad(registro.FECHA_INGRESO);
    antiguedades[rango] = (antiguedades[rango] || 0) + 1;
  });
  const agentesPorAntiguedad = Object.entries(antiguedades).map(([rango, cantidad]) => ({
    rango,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  ...
  const agentesPorGenero = Object.entries(porGenero).map(([genero, cantidad]) => ({
    genero,
    cantidad,
    porcentaje: parseFloat(calcularPorcentaje(cantidad, totalRegistros))
  }));
  ...
  // Análisis salarial
  const sueldos = datosExcel
    .map(registro => parseFloat(registro.SUELDO_BASICO) || 0)
...
router.post('/', protect, admin, upload.single('archivo'), async (req, res) => {
  try {
    ...
    for (const [secretariaNombre, datosSecretaria] of Object.entries(porSecretaria)) {
      ...
      // Crear o actualizar el análisis en la base de datos
...
router.get('/status', protect, admin, async (req, res) => {
  try {
    const ultimoAnalisis = await AnalysisData.findOne({ activo: true })
      .sort({ createdAt: -1 })
      .populate('archivoInfo.usuarioId', 'username');
    
    if (!ultimoAnalisis) {
      return res.json({ message: 'No hay datos procesados' });
    }
    
    const totalSecretarias = await AnalysisData.countDocuments({ activo: true });
    
    res.json({
      ultimoProcesamiento: ultimoAnalisis.createdAt,
      archivo: ultimoAnalisis.archivoInfo.nombreArchivo,
      usuario: ultimoAnalisis.archivoInfo.usuarioId?.username,
      totalSecretarias,
      totalRegistros: ultimoAnalisis.archivoInfo.totalRegistros,
    });
